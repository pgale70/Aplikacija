...
export class ArticleService extends TypeOrmCrudService<Article> {
    ...
    async editFullArticle(articleId: number, data: EditArticleDto): Promise<Article | ApiResponse> {
       // moramo da izvučemo postojeći artikle iz repozitorijuma 
       const existingArticle: Article = await this.article.findOne(articleId, {
           relations: [ 'articlePrices', 'articleFeatures' ]  // definišemo sa čime je sve povezana ova tabela
       });
       ...
       // provjeravamo da li se cijena mijenjala pa ako jeste upisujemo je u tabelu
       const newPriceString: string = Number(data.price).toFixed(2); // npr. 50.2 -> "50.20"
    ...
    async search(data: ArticleSearchDto): Promise<Article[]> {
        ...
        // pošto u Article namamo direktbo price onda moramo uraditi ovu vezu
        builder.innerJoinAndSelect(
            "article.articlePrices", 
            "ap",
            // Ovo nije primjer najbolje prakse!
            "ap.createdAt = (SELECT MAX(ap.created_at) FROM article_price AS ap WHERE ap.article_id = article.article_id)"

            //Pametnije rješenje (zahtijeva u bp triger_article_price_ai)
            // "ap.current = 1"
            );


        // koristimo leftJoin jer se može desiti da imamo artikal koji nema features
        builder.leftJoinAndSelect("article.articleFeatures", "af");

        builder.where('article.categoryId = :id', { id: data.categoryId }); // :categoryId je parametar upita
        ...
        //pretpostavljamo da je slaganje po imenu u rastućem poretku
        ...
        if(data.orderBy) {
            ...
            if(orderBy === 'price') {
                orderBy = 'ap.price'; // Potencijalna greška ako imamo više cijena, moraćemo da uzmemo zadnju cijenu
            }
        ...
        //koliko elemenata treba da preskočimo
        builder.skip(page * perPage);
        // koliko elemenata prikazujemo
